...
void window::show_text(string input, int x, int y, RGB color, string font_addr, int size){
	SDL_Color textColor = { static_cast<Uint8>(color.red), 
...
void window::play_music(std::string filename, int loops) {
    if (Mix_OpenAudio(44100, MIX_DEFAULT_FORMAT, 2, 2048) < 0) {
        throw std::string("SDL_mixer could not initialize! SDL_mixer Error: ") + Mix_GetError();
    }
    music = Mix_LoadMUS(filename.c_str());
    if (music == nullptr) {
        throw std::string("Failed to load music! SDL_mixer Error: ") + Mix_GetError();
    }
    if (Mix_PlayMusic(music, loops) == -1) {
        throw std::string("Failed to play music! SDL_mixer Error: ") + Mix_GetError();
    }
}
